		...
		const Reference& o, const std::vector<Object>& args)
{
	...
	string GetFieldData (const Class::FieldList& fields)
	{
		stringstream tmpl;
		tmpl << "std::vector<FieldData> data;\n";
		for (const auto& field : fields)
		{
			tmpl << "	data.push_back({\"" + field.Name + "\", \"" + field.Type + "\", " + "static_cast<AccessModifier>(" + std::to_string(static_cast<int>(field.Access)) + ")}); \n";
		}
		tmpl << "	return data;";
		return tmpl.str();
	}
...
std::vector<FieldData> Class<%name%>::GetFieldData() const
{
	%all_fields%
}
...
namespace
{
	...
	o << ReplaceAll(
			tmpl.str(),
			{
				...
				{"%all_fields%",GetFieldData(c.Fields)}
			});
}
