...
int main(int argc, char** argv)
{
	...
	else
	{
		...
		std::vector<std::unique_ptr<TypeBase>> uniqueTypes;
		for (int i = types.size()-1; i >= 0; i--)
		{
			bool hasType = false;
			for (size_t j = 0; j < uniqueTypes.size(); j++)
			{
				if (uniqueTypes[j]->GetFullName() == types[i]->GetFullName())
				{
					hasType = true;
				}
			}
			if (!hasType)
			{
				std::cout << types[i]->GetFullName() << std::endl;
		...
		serializer::Serialize(uniqueTypes, options);
	}
	return 0;
}
